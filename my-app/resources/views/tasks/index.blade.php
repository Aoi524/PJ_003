        ...
        <form action="{{ route('tasks.store')}}" method="POST" class="mb-4">
            ...
            <div class="mb-2">
                <div>
                    ...
                    <input type="text" name="task_name" id="task_name" class="border border-gray-300 w-48 mb-2">
                </div>
                <div>
                    <div>
                        <label for="due_date" class="text-xs">期限</label>
                    </div>
                    <input type="datetime-local" name="due_date" id="due_date" value="{{ now()->format('Y-m-d H:i') }}" class="border p-2 mb-2">
                </div>
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">追加</button>
            </div>
        </form>
        <div>
            <ul class="list-disc text-sm md:w-6/12">
                @foreach ($tasks as $task)
                    <li class="flex justify-between mb-2">
                        <div>{{ $task->task_name }} : <span class="mx-1">{{ date('Y-m-d H:i', strtotime($task->due_date)) }}</span></div>
                        <form action="{{ route('tasks.maskAsDelete', $task->id) }}" method="POST">
                            @csrf
                            @method('PUT')
